...
import { updateBranch } from '@/app/actions/branch';
...
export function EditBranchDialog({ branch, onBranchUpdated }: { branch: Branch, onBranchUpdated?: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const result = await updateBranch(branch._id, name, address, managerName);

        if (result.success) {
            setOpen(false);
            if (onBranchUpdated) onBranchUpdated();
        } else {
            setError(typeof result.error === 'string' ? result.error : 'Failed to update branch');
        }
        setIsLoading(false);
    };
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[425px] bg-white dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800">
                <DialogHeader>
                    <DialogTitle className="text-zinc-900 dark:text-zinc-100">Edit Branch</DialogTitle>
                </DialogHeader>
